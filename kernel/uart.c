...
static int uart_open(chardev_t *cdev)
{
  ...
  pio_disable(uart->pio_regs, uart->pio_tx);
  pio_disable(uart->pio_regs, uart->pio_rx);
  pio_set_peripheral_mode(uart->pio_regs, uart->pio_tx, uart->pio_mode);
  pio_set_peripheral_mode(uart->pio_regs, uart->pio_rx, uart->pio_mode);
  pio_enable_pullup(uart->pio_regs, uart->pio_tx);
...
static ssize_t uart_write(chardev_t *cdev, const void *src, size_t size)
{
  ...
  if (uart->write_buff == NULL)
    uart->write_buff = uart_take_buff(uart, false);
...
static void isr_handler(uart_t *uart)
{
  if (bits_set(uart->regs->sr, _b(4)) && uart_tx_int_enabled(uart)) {
    ...
    set_bits(uart->flags, UART_FLAG_TX);
    task_signal(__isr_lower_task, SIG_USR1);
  }
  if (bits_set(uart->regs->sr, _b(3)) && uart_rx_int_enabled(uart)) {
    pdc_swap_buff(&uart->pdc_rx);
    uart_disable_rx_int(uart);
    set_bits(uart->flags, UART_FLAG_RX);
    task_signal(__isr_lower_task, SIG_USR1);
  }
}
...
static void uart_handle_rx_event(uart_t *uart)
{
  ...
  clr_bits(uart->flags, UART_FLAG_RX);
  signal_wake_all(&uart->cdev.wait_list);
...
static void uart_handle_tx_event(uart_t *uart)
{
  ...
  clr_bits(uart->flags, UART_FLAG_TX);
  signal_wake_all(&uart->cdev.wait_list);

  spinlock_release(&uart->cdev.lock);
}

static void uart_signal_handler(signal_id_t sig)
{
  for (int i = 0; i < NELEMS(__uart_devs); ++i) {
    if (bits_set(__uart_devs[i].flags, UART_FLAG_RX))
      uart_handle_rx_event(&__uart_devs[i]);
    if (bits_set(__uart_devs[i].flags, UART_FLAG_TX))
...
{
  task_set_signal_handler(task_self(), uart_signal_handler, SIG_USR1);
...
void uart_init()
{
  ...
  __isr_lower_task = task_add(uart_data_handler, 1024, 0, "uart_isr");
}
